function getPositiveRandomInt(min, max) { //функция на генерацию случайного целого положительного числа
  ...
  if (min < 0) {
    const validationPositiveError = 'min = ' + min + ' - минимальное значение диапазона должно быть больше или равно 0.';
    ...
    if (min >= max) {
      ...
      min = Math.ceil(min);
      max = Math.floor(max);
      result = Math.floor(Math.random() * (max - min + 1)) + min; //включение максимума и минимума
      return result;
    }
  }
}
...
function getPositiveRandomFloat(min, max, numLength) { //функция на генерацию случайного дробного положительного числа
  ...
  if (min < 0) {
    ...
    if (min >= max) {
      ...
      //проверки пройдены, генерим число
      result = Math.random() * (max - min + 1) + min; //Максимум и минимум включаются
